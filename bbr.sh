...
startbbrmod(){
	...
	if [[ "${release}" == "centos" ]]; then
		...
		wget -N --no-check-certificate https://raw.githubusercontent.com/zxlhhyccc/Linux-NetSpeed/master/bbr/tcp_tsunami.c
...
startbbrplusmod_ubuntu18.04(){
	...
	if [[ "${release}" == "ubuntu" ]]; then
		...
		apt-get update
	        ...
	        apt-get -y install make gcc libelf-dev
		mkdir bbrmod && cd bbrmod
		wget -N --no-check-certificate https://raw.githubusercontent.com/zxlhhyccc/TCP_BBR/master/v4.19/tcp_bbrplus.c
		echo "obj-m := tcp_bbrplus.o" > Makefile
		make -C /lib/modules/$(uname -r)/build M=`pwd` modules CC=/usr/bin/gcc
		install tcp_bbrplus.ko /lib/modules/$(uname -r)/kernel
		cp -rf ./tcp_bbrplus.ko /lib/modules/$(uname -r)/kernel/net/ipv4
